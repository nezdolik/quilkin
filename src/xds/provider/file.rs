...
impl FileProvider {
    ...
    fn create_listener_resources(&self, res: &Resources) -> Result<Listener, tonic::Status> {
    ...
    #[tracing::instrument(skip(self))]
    fn create_cluster_resources(&self, res: &Resources) -> Cluster {
    ...
    async fn discovery_request(
        ...
        let value = match kind {
            ResourceType::Endpoint => {
                let cluster = self.create_cluster_resources(&rs);
                crate::prost::encode(&cluster).map_err(prost_to_tonic)?
            }
            ResourceType::Listener => {
                let listener = self.create_listener_resources(&rs)?;
...
mod tests {
    ...
    "#;
        ...
        let cluster_resource = fp.create_cluster_resources(&rs);
        let listener_resource = fp.create_listener_resources(&rs).unwrap();
